...
\begin{proof}
	...
	suppose $g^{-1}$ is the inverse of $g \in G$. Also, let $h(g) = h'$, and
	$h(g^{-1}) = h''$.
	Then, $h' h'' = h(g) h(g^{-1}) = h(g g^{-1}) = h(1_G) = 1_H$.
	And, also $h'' h' = h(g^{-1}) h(g) = h(g^{-1} g) = h(1_G) = 1_H$.
	This shows that $h'$ and $h''$ are inverses of each other, thereby
	implying $h$ preserves inverses, which is what we set out to prove.
\end{proof}
